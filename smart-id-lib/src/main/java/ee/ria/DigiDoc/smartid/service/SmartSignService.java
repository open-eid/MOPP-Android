...
public class SmartSignService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent != null) {
            @NonNull SmartIDSignatureRequest request =
                    (SmartIDSignatureRequest) intent.getSerializableExtra(SmartSignConstants.CREATE_SIGNATURE_REQUEST);

            @NonNull ArrayList<String> certificateCertBundle = intent.getStringArrayListExtra(SmartSignConstants.CERTIFICATE_CERT_BUNDLE);
            ...
            try {
                ...
                @NonNull String base64Hash = containerWrapper.prepareSignature(getCertificatePem(sessionStatusResponse.getCert().getValue()));
